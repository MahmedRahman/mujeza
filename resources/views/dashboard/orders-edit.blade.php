...
@section('content')
    <section class="card">
        ...
        <p style="margin-bottom: 14px; color: #4b5563; font-weight: 500;">
            عدل بيانات العميل والمنتجات والكميات، والإجمالي هيتحسب تلقائي.
        </p>
        ...
        <form method="POST" action="{{ route('orders.update', $order) }}">
            ...
            <div style="border:1px solid #efe3b7; border-radius:10px; padding:12px; background:#fffcf2;">
                ...
                <div style="overflow-x:auto;">
                    <table style="width:100%; border-collapse:collapse; min-width: 820px;">
                        ...
                        <tbody id="order-lines">
                            @php
                                $oldIds = old('product_ids');
                                $oldQtys = old('quantities');
                                $initialLines = [];
                                if (is_array($oldIds) && is_array($oldQtys) && count($oldIds) === count($oldQtys)) {
                                    foreach ($oldIds as $i => $id) {
                                        $initialLines[] = ['product_id' => (int) $id, 'quantity' => (int) ($oldQtys[$i] ?? 1)];
                                    }
                                } else {
                                    foreach ($order->items as $item) {
                                        $initialLines[] = ['product_id' => (int) $item->product_id, 'quantity' => (int) $item->quantity];
                                    }
                                }
                                if ($initialLines === []) {
                                    $initialLines[] = ['product_id' => (int) ($products->first()->id ?? 0), 'quantity' => 1];
                                }
                            @endphp

                            @foreach ($initialLines as $lineIndex => $line)
                                <tr class="order-line" data-index="{{ $lineIndex }}">
                                    <td style="padding:10px; border:1px solid #efe3b7;">
                                        <select name="product_ids[]" class="product-select" style="width:100%; border:1px solid #d1d5db; border-radius:8px; padding:10px; font-family:inherit;" required>
                                            @foreach ($products as $p)
                                                <option value="{{ $p->id }}" data-unit-price="{{ $p->price }}" {{ (int) $line['product_id'] === (int) $p->id ? 'selected' : '' }}>
                                                    {{ $p->title }}
                                                </option>
                                            @endforeach
                                        </select>
                                    ...
                                    <td style="padding:10px; border:1px solid #efe3b7;">
                                        <input name="quantities[]" type="number" min="1" step="1" class="quantity-input" value="{{ max(1, (int) $line['quantity']) }}" style="width:100%; border:1px solid #d1d5db; border-radius:8px; padding:10px; font-family:inherit;" required>
                                    </td>
                                    <td style="padding:10px; border:1px solid #efe3b7;">
                ...
                <div style="display:flex; justify-content:flex-end; gap: 12px; align-items:center; margin-top: 12px;">
                    ...
                    <div>
                        <input type="text" id="order-total" value="{{ number_format((float) $order->total_amount, 2, '.', '') }}" readonly style="width:180px; border:1px solid #d1d5db; border-radius:8px; padding:10px; font-family:inherit; font-weight:800;">
                    </div>
                </div>
